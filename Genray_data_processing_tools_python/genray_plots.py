...
# Modified by HanYue Zhao @ ENN, 2023
...
mpl.use('Agg') # only save, no windows
...
def plot_ray_trace(eqname,caseset,NO):
    '''
    @ Parameter
    @ eqname - equilibrium configuration
    @ caseset - icrh, ecrh, lwrh and input power
    @ NO - number of rays plotted
    
    Example : plot_ray_trace('EXL_6935_3fluid','icrh_10MHz_100kW',1)
    ...
    c0 = time.time() # total cpu time spent in the script so far
    
    #------------------------------------------
    # Specify eqdsk_name and NCfile name (filenm)
    # eqdsk_name = 'g088890.00000' + eqconfig
    # filenm     = '88890_0ms_170ghz_x_up_b-9_a30.nc' + eqconfig + '/case_' + caseset + '/exl50.nc'
    eqdsk_name = 'g088890.00000'
    filenm='EHL_2_105ghz_tf120.nc'
    print("The equilibrium file is %s, and the NC file is %s ." % (eqdsk_name, filenm))
    #------------------------------------------
    ...
    def read_vector(flnm,ndim,nums_per_line):
        ...
        if int(nlines)*nums_per_line != ndim: nlines=nlines+1
        #print nlines
        for i in range(0,int(nlines)):
    ...
    try:# Try reading eqdsk file, hoping it exists in the working directory.
        ...
        print(f'reading eqdsk file {eqdsk_name}')
    ...
    #    cols=range(16,65,16); cols.reverse()  #
        cols=[64, 48, 32, 16]
        b=' '
    ...
    ##    print '========================================'
        ...
        if ncontr > 0:
            ...
            rcontr=rzcontr[:,0]
            zcontr=rzcontr[:,1]
            #print 'rcontr,zcontr=',shape(rcontr),shape(zcontr)
        if nlimiter > 0:
            try:
    ...
    ##            print 'rlimiter,zlimiter=',shape(rlimiter),shape(zlimiter)
            except:  
    ...
    if Rmax_plot==0:
        ...
        if n_wall > 5:  # data on wall is present
            Rmax=np.amax(r_wall)*100
            Rmin=np.amin(r_wall)*100
            Zmax=np.amax(z_wall)*100
            Zmin=np.amin(z_wall)*100
            print('Data on wall: Rmin,Rmax=',Rmin,Rmax)
        if ncontr > 5:  # data on LCFS is present
            R_LCFS_max=np.amax(rcontr)*100
            R_LCFS_min=np.amin(rcontr)*100
            Z_LCFS_max=np.amax(zcontr)*100
            Z_LCFS_min=np.amin(zcontr)*100
            print('Data on LCFS: R_LCFS_min,R_LCFS_max=',R_LCFS_min,R_LCFS_max)
        if nlimiter >5: # data on limiter is present
            Rmax=np.amax(rlimiter)*100
            Rmin=np.amin(rlimiter)*100
            Zmax=np.amax(zlimiter)*100
            Zmin=np.amin(zlimiter)*100
            print('Data on limiter: Rmin,Rmax=',Rmin,Rmax)
    ...
    #plt.title('$ T_e, T_i $')
    #plt.xlabel(r'$\rho$')
    plt.title('$ Te $   $ (keV) $',fontsize=fnt+4,y=1.03)
    plt.xlabel(r'$\rho$',fontsize=fnt+4)
    plt.grid(True)
    for i in range(0,Nsp,1):
        if remainder(i,4)==0: col='b'
    ...
    savefig('001genray_profiles_T-n.png',format='png') # try pdf,eps,ps
    # show() 
    #stop
    ...
    #--------------------------------------------------------------------------
    fig4=plt.figure(figsize=(12,8))  # RAYS in top-view (R-phi or X-Y)
    ...
    plt.xlabel('$X$  $(cm)$')
    plt.ylabel('$Y$  $(cm)$')
    circ1 = plt.Circle((0.0, 0.0), radius=80, ec='r',lw=2.0,ls=':',fc='w')
    ax.add_patch(circ1)
    circ3 = plt.Circle((0.0, 0.0), radius=40, ec='r',lw=2.0,ls=':',fc='w')
    ax.add_patch(circ3)
    circ2 = plt.Circle((0.0, 0.0), radius=20, ec='r',lw=2.0,ls=':',fc='w')
    ax.add_patch(circ2)
    ...
    plt.plot(bndry_in_X,  bndry_in_Y,  'b',linewidth=linw*3)
    plt.plot(bndry_out_X, bndry_out_Y, 'b',linewidth=linw*3)
    
    plt.axis('equal')  
    plt.axis('off')
    plt.savefig('002genray_rays_in_R-phi.png') 
    # show()
    #stop
    
    
    #--------------------------------------------------------------------------
    fig0=plt.figure(figsize=(12,8)) # RAYS in cross-sectional view R-Z
    ax = plt.subplot(111)
    ax.set_aspect(1.0)   
    ...
    plt.ylabel('$Z$  $(cm)$')
    ##plt.grid(True)
    line = mpl.lines.Line2D([40.0, 40.0], [-120.0, 120.0],marker="None",ms=10.0,
                        lw=2.5,ls=':',color='black', axes=ax)
    ax.add_line(line)
    line = mpl.lines.Line2D([20.0, 20.0], [-120.0, 120.0],marker="None",ms=10.0,
                        lw=2.5,ls=':',color='black', axes=ax)
    ax.add_line(line)
    
    line = mpl.lines.Line2D([78.0, 78.0], [-120.0, 120.0],marker="None",ms=10.0,
                        lw=2.5,ls=':',color='black', axes=ax)
    ax.add_line(line)
    ...
    if ncontr > 0:  # plot LCFS, if any
        plt.plot(multiply(rcontr,100),multiply(zcontr,100),'r',linewidth=linw*2)
    ...
    ##if n_wall+ncontr+nlimiter ==0: # no data on walls or LCFS; Try plotting PSI
        if n_eqdsk==1:
            R,Z = np.meshgrid(np.array(eqdsk_r), np.array(eqdsk_z))
    ...
    ##        CS=plt.contour(Req,Zeq,PSI,levels2,linewidth=2,cmap=plt.cm.jet)
            
            CS=plt.contour(Req,Zeq,PSI,50,linewidth=2,cmap=plt.cm.jet)
    ...
    plt.savefig('003genray_rays_inRZ.png') 
    # show()
    ...
    #--------------------------------------------------------------------------
    #--------------------------------------------------------------------------
    fig3=plt.figure(figsize=(12,8))  # w/wc resonances and Pol.Flux in cross-section view R-Z
    ax = plt.subplot(1, 1, 1)
    ax.set_aspect(1.0)
    ax.axis([xmin,xmax,zmin,zmax])
    ##plt.hold(True)
    ...
    plt.savefig('004genray_wwc_inRZ.png') 
    # show()
    #stop
    
    
    #--------------------------------------------------------------------------
    fig8=plt.figure(figsize=(12,8))  # E-wave-field along RAYS vs pol.distance(t)
    plt.subplot(231) #-------------------------
    ##plt.hold(True)
    ...
    plt.xlabel('$poloidal$ $distance$ $along$ $ray$  $(cm)$')
    plt.savefig('005genray_rays_Ewave_p.png') 
    # show()
    
    
    #--------------------------------------------------------------------------
    fig81=plt.figure(figsize=(12,8))  # Vgroup/c along RAYS vs pol.distance(t)
    plt.subplot(231) #-------------------------
    ##plt.hold(True)
    plt.grid(True)
    ...
    plt.xlabel('$poloidal$ $distance$ $along$ $ray$  $(cm)$')
    plt.savefig('006genray_rays_Vgroup_p.png') 
    # show()
    
    
    
    #--------------------------------------------------------------------------
    fig5=plt.figure(figsize=(12,8))  # wce/w and (wpe/w)^2 along RAYS vs distance
    qm=  charge[0]/mass[0]  # First species is electrons
    q2m= charge[0]**2/mass[0]
    plt.subplot(231) #-------------------------
    ##plt.hold(True)
    plt.grid(True)
    plt.title('$|\omega_{ce}/\omega|$',y=1.03)
    xticks(np.arange(ws_min, ws_max, step=ws_step) )
    plt.xlim( (ws_min, ws_max) )
    for i in range(0,Nrays,1):  # i goes from 0 to Nrays-1
        if remainder(i,6)==0: col='b'
    ...
    ##    plt.hold(True)
        ...
        xticks(np.arange(ws_min, ws_max, step=ws_step) )
        plt.xlim( (ws_min, ws_max) )
    plt.savefig('007genray_rays_wc_wp_p.png') 
    # show()
    
    
    
    #--------------------------------------------------------------------------
    fig5=plt.figure(figsize=(12,8))  # Refractive indices along RAYS vs R(t)
    ax=plt.subplot(231) #-------------------------
    ##plt.hold(True)
    ...
    locs, labels = plt.xticks() 
    plt.setp(labels, rotation=90)
    for i in range(0,Nrays,1):  # i goses from 0 to Nrays-1
        if remainder(i,6)==0: col='b'
        if remainder(i,6)==1: col='g'
        if remainder(i,6)==2: col='r'
        if remainder(i,6)==3: col='c'    
    ...
    plt.savefig('008genray_rays_refr-index_R.png') 
    # show()
    
    
    
    #--------------------------------------------------------------------------
    fig6=plt.figure(figsize=(12,8))  # Refractive indices along RAYS vs pol.distance(t)
    plt.subplot(231) #-------------------------
    ##plt.hold(True)
    plt.title('$n_R$',y=1.03)
    xticks(np.arange(ws_min, ws_max, step=ws_step) )
    ...
    plt.savefig('009genray_rays_refr-index_p.png') 
    # show()
    ...
    #--------------------------------------------------------------------------
    fig7=plt.figure(figsize=(12,8))   # Power profiles
    plt.subplot(221) #-------------------------
    ...
    ###### 找到最大功率密度处的rho ######
    # 1. 找到最大功率密度所在的索引
    max_idx = np.argmax(powden_e)
    # 2. 获取对应的 rho 值 (x轴) 和 功率值 (y轴)
    rho_peak = rho_bin_center[max_idx]
    pow_peak = powden_e[max_idx]
    # 3. 在图上标记点 (画一个黑色的圆点)
    plt.plot(rho_peak, pow_peak, 'ko', markersize=6) 
    # 4. 画一条垂直虚线指向 x 轴，方便读数
    plt.vlines(rho_peak, 0, pow_peak, colors='k', linestyles='--', linewidth=1)
    # 5. 添加文本标注
    # xy是点的位置，xytext是文字的位置，arrowprops用来画箭头
    plt.annotate(f'$\\rho_{{dep}}={rho_peak:.3f}$', 
             xy=(rho_peak, pow_peak), 
             xytext=(rho_peak + 0.1, pow_peak),
             arrowprops=dict(facecolor='black', arrowstyle='->'),
             fontsize=14)
    # 增加安全性检查
    y_max_val = np.amax(powden_e)
    if np.isnan(y_max_val) or y_max_val <= 0:
        y_max_val = 1.0 # 给个默认值防止报错
    axis([0., rhomax, 0., 1.05 * y_max_val + 1.e-2])
    ...
    absorb_percent = power_total * 100 / power_inj_total
    plt.suptitle(f'absorb percent: {absorb_percent} %')

    savefig('010genray_profiles_power.png')
    # show() 
    
    
    
    #--------------------------------------------------------------------------
    fig1=plt.figure(figsize=(12,8))   # Current profiles
    plt.subplot(221) #-------------------------
    ...
    plt.grid(True)
    plt.xlabel(r'$\rho$')
    plt.title('$<j_{||}>B_{pol}($'+r'$\theta$'+'$=0)/<B>$',verticalalignment='center',y=1.03)
    plt.plot(rho_bin_center,s_cur_den_poloidal,'r',linewidth=linw)
    savefig('011genray_profiles_J.png')
    # show() 
    
    
    fig7=plt.figure(figsize=(12,8))   # Pe and J|| profiles
    #----------------
    plt.subplot(221) #------------------------- Pe
    plt.ylabel('$p_e$ $(kW/m^3)$') 
    plt.grid(True)
    ...
    plt.subplot(223) #------------------------- J||
    txt="$I_{CD}$"+"$=$%3.3f" %(parallel_cur_total) +" $kA$"
    title(txt,x=1.5)
    plt.grid(True)
    ...
    plt.xlabel('$pol.distance$ $along$ $ray$  $(cm)$')
    plt.savefig('013genray_rays_delpwr.png') 
    # plt.savefig('013genray_rays_delpwr.eps') 
    # show()
    ...
    cpu_time = time.time() - c0
    print( 'elapsed and cpu time since start (sec.) =', elapsed_time, cpu_time)
    print( 'FINISHED')
    
    
    return 1   #end function plot_raytrace(eqname,wavetype)
    
    
#----------------------------------------------------------------------------------------------------------------------    
if __name__=='__main__':
    # 
    # read [eqname,caseset,NO] from input arguments (argv = [commond_name eqname,caseset,NO])
    args       = sys.argv
    if (len(sys.argv) >= 2):
        eqname     = args[1]
    else: # default option
    ...
    plot_ray_trace(eqname,caseset,NO)
